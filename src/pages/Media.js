...
export default function Media() {
  ...
  // Preloaded fallback content (direct image + embed links)
  const fallbackItems = [
    {
      type: "image",
      src: "https://images.unsplash.com/photo-1588776814546-ec7d3f3f3f3d?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80",
      title: "Health Camp",
      caption: "Free medical checkups in rural Karnataka villages",
    },
    {
      type: "image",
      src: "https://images.unsplash.com/photo-1509099836639-18ba106728d4?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80",
  ...
  return (
    <div className="container py-4">
      <h2 className="mb-4 text-center"> Media Showcase</h2>
        ...
        <ul className="list-group">
          {items.filter(i => i.type === "press").map((press, idx) => (
            <li key={idx} className="list-group-item d-flex justify-content-between align-items-center">
              {press.title}
              <a href={press.src} target="_blank" rel="noopener noreferrer" className="btn btn-sm btn-outline-secondary">
                View
              </a>
            </li>
          ))}
      ...
      <section>
        ...
        <ul className="list-group">
          {items.filter(i => i.type === "blog").map((blog, idx) => (
            <li key={idx} className="list-group-item d-flex justify-content-between align-items-center">
              {blog.title}
              <a href={blog.src} target="_blank" rel="noopener noreferrer" className="btn btn-sm btn-outline-primary">
                Read
              </a>
            </li>
          ))}
